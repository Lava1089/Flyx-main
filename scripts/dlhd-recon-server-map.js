#!/usr/bin/env node
/**
 * DLHD Server Map Verification - February 28, 2026
 * Checks if the hardcoded server map is still accurate
 * Tests a sample of channels from each server
 */

async function testChannel(ch, expectedServer) {
  const domain = 'adsfadfds.cfd';
  const url = `https://chevy.${domain}/proxy/${expectedServer}/premium${ch}/mono.css`;
  try {
...
// Current server map from fast-extractor.ts
...
async function main() {
  console.log('=== DLHD SERVER MAP VERIFICATION ===\n');
  ...
  // Test a sample from each server (5 channels per server)
  for (const [server, channels] of Object.entries(SERVER_CHANNELS)) {
    totalChannels += channels.length;
    mappedChannels += channels.length;
    
    // Pick 5 evenly spaced channels to test
    const step = Math.max(1, Math.floor(channels.length / 5));
    const sample = [];
    for (let i = 0; i < channels.length && sample.length < 5; i += step) {
      sample.push(channels[i]);
    }
    
    console.log(`Testing ${server} (${channels.length} channels, sample: ${sample.join(',')})`);
    
    for (const ch of sample) {
      const works = await testChannel(ch, server);
      if (works) {
        correctMappings++;
        process.stdout.write(`  ch${ch}: OK  `);
      } else {
        // Try to find the actual server
        const actual = await findWorkingServer(ch);
        wrongMappings++;
        wrongList.push({ ch, expected: server, actual });
        process.stdout.write(`  ch${ch}: WRONG (actual: ${actual || 'NONE'})  `);
      }
    }
    console.log('');
  }
  
  // Check for unmapped channels (851-900 range)
  console.log('\n--- Checking for NEW channels (851-870) ---');
  for (let ch = 851; ch <= 870; ch++) {
    const server = await findWorkingServer(ch);
    if (server) {
      console.log(`ch${ch}: FOUND on ${server} (NOT IN MAP!)`);
    }
  }
  
  console.log('\n=== RESULTS ===');
  console.log(`Total mapped channels: ${mappedChannels}`);
  console.log(`Tested: ${correctMappings + wrongMappings}`);
  console.log(`Correct: ${correctMappings}`);
  console.log(`Wrong: ${wrongMappings}`);
  if (wrongList.length > 0) {
    console.log('\nWrong mappings:');
    for (const w of wrongList) {
      console.log(`  ch${w.ch}: expected=${w.expected}, actual=${w.actual || 'OFFLINE'}`);
    }
  }
}
