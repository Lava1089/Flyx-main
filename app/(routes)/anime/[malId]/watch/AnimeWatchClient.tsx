...
export default function AnimeWatchClient() {
  ...
  // Provider state
  const [currentProvider, setCurrentProvider] = useState<'animekai' | 'hianime' | 'vidsrc' | '1movies' | 'flixer' | 'vidlink' | 'hexa' | undefined>(undefined);
  const [availableProviders, setAvailableProviders] = useState<Array<'animekai' | 'hianime' | 'vidsrc' | '1movies' | 'flixer' | 'vidlink' | 'hexa'>>([]);
  ...
  const handleProviderChange = useCallback(async (_provider: 'animekai' | 'hianime' | 'vidsrc' | '1movies' | 'flixer' | 'vidlink' | 'hexa', currentTime: number = 0) => {
    setMobileResumeTime(currentTime);
