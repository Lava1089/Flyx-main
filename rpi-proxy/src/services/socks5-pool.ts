...
import net from 'net';
import tls from 'tls';
import type { Socks5Proxy, Socks5PoolConfig, Socks5PoolState, ProxySelection } from '../types';
...
const DEFAULT_CONFIG: Socks5PoolConfig = {
  minPoolSize: 100,
...
async function fetchProxyLists(config: Socks5PoolConfig): Promise<string[]> {
...
function validateSocks5Proxy(proxyStr: string, timeoutMs: number): Promise<boolean> {
...
async function validateProxiesBatch(proxies: string[], config: Socks5PoolConfig): Promise<string[]> {
  ...
  for (let i = 0; i < proxies.length; i += concurrency) {
    ...
    for (const r of batchResults) {
      if (r.valid) results.push(r.proxyStr);
    }
    if (results.length >= config.minPoolSize * 1.5) break;
  }
...
export async function refreshProxyPool(config: Socks5PoolConfig = DEFAULT_CONFIG): Promise<void> {
  if (pool.validating) return;
...
export function getNextProxy(): ProxySelection {
...
export function markProxyFailed(proxyStr: string): void {
...
export function startPoolRefresh(config: Socks5PoolConfig = DEFAULT_CONFIG): void {
  // Initial refresh
...
export function stopPoolRefresh(): void {
  if (refreshTimer) {
