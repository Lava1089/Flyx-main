...
exports.STREAM_PROXY_PATHS = void 0;
 ...
 */
function createStreamProxyHandler(path) {
    const config = STREAM_CONFIGS[path];
    if (!config)
        throw new Error(`No stream config for ${path}`);
    return async function handleStreamProxy(req, res) {
        ...
        const chunks = [];
        let spawned = true;
        rust.stdout.on('data', (c) => chunks.push(c));
        rust.stderr.on('data', (d) => {
            const msg = d.toString().trim();
            if (msg)
                console.log(`[${config.label}] stderr: ${msg.substring(0, 200)}`);
        });
        rust.on('error', (err) => {
            spawned = false;
            console.warn(`[${config.label}] rust-fetch not available: ${err.message}`);
            if (!res.headersSent) {
                (0, utils_1.sendJsonError)(res, 502, { error: 'rust-fetch not available', details: err.message, timestamp: Date.now() });
            }
        });
        rust.on('close', (code) => {
            if (!spawned)
            ...
            if (code !== 0) {
                ...
                if (!res.headersSent) {
                    (0, utils_1.sendJsonError)(res, 502, { error: `rust-fetch exit ${code}`, timestamp: Date.now() });
                }
                return;
            }
...
exports.STREAM_PROXY_PATHS = Object.keys(STREAM_CONFIGS);
