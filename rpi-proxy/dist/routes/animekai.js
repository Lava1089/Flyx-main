...
function proxyAnimeKaiStream(targetUrl, customUserAgent, customReferer, customOrigin, customAuth, res) {
    ...
    if (url.pathname.startsWith('/_v')) {
        ...
        fetch(targetUrl, {
            ...
            pump().catch(() => { if (!res.writableEnded)
                res.end(); });
        })
            .catch((err) => {
            (0, utils_1.sendJsonError)(res, 502, { error: 'MegaCloud CDN fetch error', details: err.message, timestamp: Date.now() });
        });
        return;
    }
    ...
    proxyReq.on('error', (err) => {
        (0, utils_1.sendJsonError)(res, 502, { error: 'AnimeKai proxy error', details: err.message, timestamp: Date.now() });
    });
    proxyReq.on('timeout', () => {
        proxyReq.destroy();
        (0, utils_1.sendJsonError)(res, 504, { error: 'AnimeKai stream timeout', timestamp: Date.now() });
    });
