...
function getCachedSession() {
    if (_cachedSession && Date.now() - _cachedSession.whitelistedAt < SESSION_TTL_MS)
        return _cachedSession;
    _cachedSession = null;
    return null;
}
function cacheSession(sessionId, proxyUrl) {
...
async function handleDLHDKeyV6(req, res) {
    ...
    if (!validKey) {
        ...
        try {
            ...
            const { stdout: recapOut } = await execFileAsync('rust-fetch', ['--mode', 'recaptcha-v3', '--site-key', siteKey, '--action', action, '--url', pageUrl, '--timeout', '8'], { timeout: 10000, windowsHide: true });
